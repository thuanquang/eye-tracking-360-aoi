...
const modernJob = JSON.parse(readFileSync(new URL('../runpod-aoi-upload/jobs/modern_01m00s-01m30s.json', import.meta.url), 'utf8'));
...
test('RunPod automatic taxonomy expands by fixed study video scene', () => {
  ...
  assert.match(script, /"thap_ba": \[/);
  assert.match(script, /"tam_coc": \[/);
  assert.match(script, /"modern": \[/);
...
test('Modern 3D RunPod job exports flat top-bottom video-space AOIs', () => {
  assert.equal(modernJob.video.name, 'modern_01m00s-01m30s.mp4');
  assert.equal(modernJob.video.projection, 'flat');
  assert.equal(modernJob.video.stereoLayout, 'top-bottom');
  assert.equal(modernJob.video.stereoEye, 'top-left');
});
