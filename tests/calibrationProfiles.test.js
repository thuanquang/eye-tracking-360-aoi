...
test('exposes conservative standard and dense research calibration profiles', () => {
  assert.equal(getCalibrationProfile('standard').calibrationPoints.length, 9);
...
test('keeps the default WebGazer training budget conservative', () => {
  ...
  assert.equal(standard.samplesPerPoint, 2);
  assert.equal(research39.samplesPerPoint, 2);
  assert.equal(research78.samplesPerPoint, 1);
  assert.equal(standard.calibrationPoints.length * standard.samplesPerPoint, 18);
  assert.equal(research39.calibrationPoints.length * research39.samplesPerPoint, 78);
  assert.equal(research78.calibrationPoints.length * research78.samplesPerPoint, 78);
});

test('resolves profile-specific calibration sample budgets with fallback', () => {
  assert.equal(getCalibrationSamplesPerPoint('standard', 12), 2);
  assert.equal(getCalibrationSamplesPerPoint('research-39', 12), 2);
  assert.equal(getCalibrationSamplesPerPoint('research-78', 12), 1);
