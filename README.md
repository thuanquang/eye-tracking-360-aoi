...
`namedAoiMetrics.session` includes total samples, total duration, total AOI-mapped fixations, average fixation duration, average number of AOIs fixated, AOI coverage percent, an MVP-level overall processing efficiency score based on time spent in AOIs, and exploratory `averageSaccadeDurationMs` values summarized from `namedAoiMetrics.transitions`. Transitions are webcam-derived fixation-gap estimates for scanpath debugging and pilot comparison; they are not validated saccade physiology. Dispersion-based fixation metrics are still bounded by webcam accuracy and should be validated in a pilot before being treated as final research measures; fallback fixation metrics are approximations for older exports.
